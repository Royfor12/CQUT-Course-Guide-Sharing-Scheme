...
#define TX_FIFO_NAME "server_to_client"
#define RX_FIFO_NAME "client_to_server"
...
int main(void)
{
    ...
    printf("Fininshed to open all fifos.\n");

    char tx_buffer[256], rx_buffer[256];
    while (true) {
        printf("Input message: ");
        fflush(stdin);
        __attribute__((unused)) int ret = scanf("%s", tx_buffer);
        int size = write(tx_fifo, tx_buffer, strlen(tx_buffer));
        ...
        size = read(rx_fifo, rx_buffer, sizeof rx_buffer / sizeof rx_buffer[0] - 1);
        rx_buffer[size] = '\0';
